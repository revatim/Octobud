...
public class PoemsActivity extends AppCompatActivity {
    ...
    AppCompatActivity activity = PoemsActivity.this;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String level = databaseHelper.getPoemsD(email);
        Log.i("hi",level);
        ...
        if (count == 1)
        {
            videoView.setVideoPath("android.resource://" + getPackageName() + "/" + R.raw.easypoems);
            videoView.start();
            //  video.start();
        }
    }
